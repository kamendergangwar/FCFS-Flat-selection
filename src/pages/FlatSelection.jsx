...
/* ───────────── helpers ───────────── */
const formatPrice = (price, formatNumber) => `₹${formatNumber(price)}`;
const getStatusTone = (status, isSel, t) => {
  if (isSel) return { fill: '#3b82f6', stroke: '#1d4ed8', glow: 'rgba(59,130,246,.35)', chipCls: 'bg-blue-100 text-blue-700 border-blue-200', label: t('Selected') };
  if (status === 'available') return { fill: '#22c55e', stroke: '#15803d', glow: 'rgba(34,197,94,.25)', chipCls: 'bg-emerald-100 text-emerald-700 border-emerald-200', label: t('Available') };
  if (status === 'held') return { fill: '#60a5fa', stroke: '#2563eb', glow: 'rgba(96,165,250,.25)', chipCls: 'bg-blue-100 text-blue-700 border-blue-200', label: t('Held') };
  return { fill: '#ef4444', stroke: '#dc2626', glow: 'rgba(239,68,68,.25)', chipCls: 'bg-rose-100 text-rose-700 border-rose-200', label: t('Sold') };
};
...
const FlatSelection = () => {
    ...
    <svg viewBox="0 0 320 390" className="w-full" style={{ maxWidth: maxW, margin: '0 auto', display: 'block' }}>
      <defs>
        {selFloorData.flats.map((flat) => {
          const tone = getStatusTone(flat.status, flat.id === selFlatId, t);
          return (
      ...
      <text x="160" y="182" textAnchor="middle" fontSize="8" fill={isDark ? '#475569' : '#94a3b8'} fontWeight="700" letterSpacing=".1em">{t('CORRIDOR')}</text>
      <rect x="140" y="340" width="40" height="26" rx="8" fill={isDark ? '#334155' : '#cbd5e1'} />
      <text x="160" y="357" textAnchor="middle" fontSize="7" fill={isDark ? '#e2e8f0' : '#475569'} fontWeight="700">{t('LIFT')}</text>
      {selFloorData.flats.map((flat) => {
        const isSel = flat.id === selFlatId;
        const isHov = flat.id === hoverFlatId;
        const tone = getStatusTone(flat.status, isSel, t);
        ...
        return (
          <g key={flat.id} onClick={() => isVis && pickFlat(flat)} onMouseEnter={() => setHoverFlatId(flat.id)} onMouseLeave={() => setHoverFlatId(null)} className={isVis ? 'cursor-pointer' : 'pointer-events-none opacity-20'} style={{ transition: 'opacity .25s' }}>
            ...
            {isSel && <text x={flat.slot.x + 22} y={flat.slot.y + 15} textAnchor="middle" fontSize="6" fill="#fff" fontWeight="800">{t('ACTIVE')}</text>}
            <text x={cx} y={flat.slot.y + 22} textAnchor="middle" fontSize="9" fill="#fff" fontWeight="800">{flat.label}</text>
            <text x={cx} y={flat.slot.y + 36} textAnchor="middle" fontSize="8" fill="rgba(255,255,255,.9)" fontWeight="700">{t(flat.type)}</text>
            <text x={cx} y={flat.slot.y + 48} textAnchor="middle" fontSize="7" fill="rgba(255,255,255,.75)" fontWeight="600">{flat.area} {t('sq.ft')}</text>
            <rect x={cx - 28} y={flat.slot.y + 53} width="56" height="13" rx="6.5" fill="rgba(0,0,0,.22)" />
            <text x={cx} y={flat.slot.y + 62.5} textAnchor="middle" fontSize="6.5" fill="#fff" fontWeight="700">{formatPrice(flat.price, formatNumber)}</text>
          </g>
        );
      })}
    </svg>
  ...
  return (
    <ApplicationLayout stepNumber={11} title={t('Flat Selection')} hideDefaultActions hideSectionHeader fullWidth>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className={`text-lg font-extrabold ${isDark ? 'text-white' : 'text-slate-900'}`}>
                  {t('Find your dream flat')}
                  <span className={`ml-2 text-sm font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>— {t('FCFS Flat Booking')}</span>
                </h2>
                <Breadcrumb items={[t(selProject.name), selBuilding.name, `${t('Floor')} ${selFloorData.number}`, selFlat?.id || '—']} isDark={isDark} />
              </div>
            </div>
            <div className="flex items-center gap-3">
              {[
                { label: t('Available'), value: projStats.available, color: 'text-emerald-500' },
                { label: t('Held'), value: projStats.held, color: 'text-blue-400' },
                { label: t('Sold'), value: projStats.sold, color: 'text-rose-400' },
              ].map((s) => (
                <div key={s.label} className={`flex items-center gap-2 rounded-xl px-3 py-2 ${isDark ? 'bg-white/5 border border-slate-700/50' : 'bg-white/80 border border-slate-200'}`}>
                  <span className={`text-lg font-black ${s.color}`}>{formatNumber(s.value)}</span>
          ...
          <div className="space-y-5">
            ...
            <section className={sectionCls}>
              <div className={`flex flex-col gap-3 border-b px-4 py-3.5 sm:flex-row sm:items-center sm:justify-between sm:px-5 ${isDark ? 'border-slate-700/60' : 'border-slate-200/80'}`}>
                <div className="flex items-center gap-3">
                  ...
                  <h3 className={`text-base font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{t('Choose Project')}</h3>
                </div>
                <div className={`flex flex-wrap items-center gap-3 text-[10px] font-semibold ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                  <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-emerald-500" />{t('Open')}</span>
                  <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-blue-500" />{t('Held')}</span>
                  <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-rose-500" />{t('Sold')}</span>
                </div>
              </div>
              ...
              <div className={`grid gap-3 border-t p-4 md:grid-cols-[1.2fr_1fr] ${isDark ? 'border-slate-700/60 bg-slate-950/50' : 'border-slate-200/80 bg-slate-50/50'}`}>
                {focusProject ? (
                  <>
                    <div>
                      <h4 className={`text-lg font-extrabold ${isDark ? 'text-white' : 'text-slate-900'}`}>{t(focusProject.name)}</h4>
                      <p className={`mt-0.5 text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{focusProject.locality}</p>
                      <p className={`mt-2 text-sm leading-relaxed ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>{t(focusProject.preview)}</p>
                    </div>
                    <div className="flex flex-col gap-2">
                      {[{ label: t('Best Fit'), value: t(focusProject.fit) }, { label: t('Commute'), value: t(focusProject.commute) }].map((d) => (
                        <div key={d.label} className={`rounded-lg p-2.5 ${isDark ? 'bg-slate-900/80 border border-slate-700/50' : 'bg-white border border-slate-200'}`}>
                          <p className={labelCls}>{d.label}</p>
                          <p className={`mt-0.5 text-xs leading-5 ${isDark ? 'text-slate-200' : 'text-slate-700'}`}>{d.value}</p>
                        </div>
                      ))}
                    </div>
                  </>
                  ...
                  <div className="col-span-full flex items-center gap-3">
                    ...
                    <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{t('Hover or click a project marker on the map to see details here.')}</p>
                  </div>
                )}
              </div>
            </section>
            ...
            <section className={sectionCls}>
              <div className={`flex flex-col gap-3 border-b px-4 py-3.5 sm:px-5 lg:flex-row lg:items-center lg:justify-between ${isDark ? 'border-slate-700/60' : 'border-slate-200/80'}`}>
                <h3 className={`text-base font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{t('Select Your Unit')}</h3>
                <div className="flex flex-wrap items-center gap-2">
                  {[
                    { label: selBuilding.name.replace('Building ', ''), step: '1', done: true },
                    { label: `${t('Floor')} ${selFloorData.number}`, step: '2', done: true },
              ...
              <div className="space-y-4 p-4 sm:p-5">
                ...
                <div>
                  <div className="mb-2 flex items-center gap-2">
                    ...
                    <span className={`text-xs font-semibold ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>{t('Building')}</span>
                  </div>
                    ...
                    {selProject.buildings.map((b) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <p className={`mt-0.5 text-[11px] ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{t('{count} open', { count: formatNumber(avail) })}</p>
                        </button>
                      );
                    })}
                  </div>
                ...
                <div>
                  <div className="mb-2 flex items-center gap-2">
                    ...
                    <span className={`text-xs font-semibold ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>{t('Floor')}</span>
                  </div>
                    ...
                    {filteredFloors.map((f) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <p className={`text-[10px] ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{t('{count} open', { count: formatNumber(avail) })}</p>
                        </button>
                      );
                    })}
                  </div>
                      ...
                      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                        <div className={cardCls + ' p-3'}>
                          <div className="flex items-center justify-between">
                            <span className={`text-xs font-semibold ${isDark ? 'text-slate-200' : 'text-slate-700'}`}><IndianRupee className="mr-1 inline h-3 w-3" />{t('Budget')}</span>
                        ...
                        <div className={cardCls + ' p-3'}>
                          <span className={`text-xs font-semibold ${isDark ? 'text-slate-200' : 'text-slate-700'}`}>{t('Floor Band')}</span>
                          <select value={floorFilter} onChange={(e) => setFloorFilter(e.target.value)} className={`mt-1.5 w-full rounded-lg border px-2.5 py-2 text-xs font-medium outline-none ${isDark ? 'border-slate-700/60 bg-slate-950 text-slate-100' : 'border-slate-200 bg-white text-slate-800'}`}>
                            ...
                            <option value="mid">{t('Mid (4-7)')}</option>
                            <option value="high">{t('High (8-10)')}</option>
                          </select>
                        </div>
                        <div className={cardCls + ' p-3'}>
                          <span className={`text-xs font-semibold ${isDark ? 'text-slate-200' : 'text-slate-700'}`}>{t('Area Type')}</span>
                          <select value={areaFilter} onChange={(e) => setAreaFilter(e.target.value)} className={`mt-1.5 w-full rounded-lg border px-2.5 py-2 text-xs font-medium outline-none ${isDark ? 'border-slate-700/60 bg-slate-950 text-slate-100' : 'border-slate-200 bg-white text-slate-800'}`}>
                            ...
                            <option value="compact">{t('Compact ≤650')}</option>
                            <option value="family">{t('Family 650-850')}</option>
                            <option value="premium">{t('Premium 850+')}</option>
                          </select>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-sm font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>
                        {t('Floor')} {selFloorData.number} — {selBuilding.name.replace('Building ', '')} {t('Tower')}
                      </span>
                    </div>
                    <div className="flex items-center justify-between gap-2 sm:justify-end">
                      <span className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{t('{count} units', { count: formatNumber(filteredFlats.length) })}</span>
                    ...
                    <div className="grid content-start gap-2 sm:grid-cols-2">
                          ...
                          >
                            <div className="flex items-start justify-between gap-2">
                              <div>
                                ...
                                <p className={`text-[11px] ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{t(flat.type)} · {flat.area} {t('sq.ft')}</p>
                              </div>
                              <span className={`rounded-md border px-1.5 py-0.5 text-[9px] font-bold ${tone.chipCls}`}>{tone.label}</span>
                            </div>
                            <div className="mt-2 flex items-center justify-between">
                              <span className={`text-xs font-bold ${isDark ? 'text-slate-100' : 'text-slate-800'}`}>{formatPrice(flat.price, formatNumber)}</span>
                      ...
                      {filteredFlats.length === 0 && (
                        <div className={`col-span-full rounded-xl border border-dashed p-6 text-center ${isDark ? 'border-slate-700/40' : 'border-slate-300'}`}>
                          <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{t('No units match your filters.')}</p>
                        </div>
                      )}
                    </div>
                  </div>
          ...
          <aside className="xl:sticky xl:top-6 xl:self-start">
            <div className={`overflow-hidden rounded-2xl border ${isDark ? 'border-slate-700/60 bg-gradient-to-b from-slate-900 via-[#0c1f38] to-slate-950' : 'border-slate-200 bg-gradient-to-b from-white via-slate-50 to-sky-50'}`}>
              ...
              <div className={`p-4 ${isDark ? 'text-white' : 'text-slate-900'}`}>
                <p className={labelCls}>{t('Booking Summary')}</p>
                ...
                <div className={`mt-3 rounded-xl border p-3.5 transition-colors duration-500 ${
                  ...
                  <p className={`flex items-center gap-1.5 text-[10px] font-semibold ${isDark ? 'text-cyan-200/80' : 'text-sky-700'}`}>
                    <Timer className="h-3 w-3" /> {t('Reservation Timer')}
                  </p>
                  ...
                  <p className={`mt-1.5 text-[10px] ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                    {isExpired ? t('Timer expired — please select again.') : selFlat ? t('Unit held while you review.') : t('Select a unit to start timer.')}
                  </p>
                </div>
                ...
                <div className="mt-3 space-y-1.5">
                  {[
                    { label: t('Project'), value: t(selProject.name), sub: selProject.locality },
                    { label: t('Building'), value: selBuilding.name },
                    { label: t('Floor'), value: `${t('Floor')} ${selFloorData.number}` },
                  ].map((item) => (
                ...
                <div className={`mt-3 rounded-xl border p-3.5 ${isDark ? 'border-slate-700/50 bg-white/[.04]' : 'border-slate-200 bg-white/80'}`}>
                  <p className={`flex items-center gap-1.5 text-[10px] font-semibold ${isDark ? 'text-cyan-200/80' : 'text-sky-700'}`}>
                    <Home className="h-3 w-3" /> {t('Selected Flat')}
                  </p>
                  {selFlat ? (
                    <div className="mt-2 space-y-2">
                      <div className={`rounded-lg p-3 ${isDark ? 'bg-slate-950/40' : 'bg-slate-100'}`}>
                        ...
                        <p className={`mt-0.5 text-xs ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>{t(selFlat.type)} · {selFlat.area} {t('sq.ft')} · {t(selFlat.facing)}</p>
                      </div>
                      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                        <div className={`rounded-lg p-2.5 ${isDark ? 'bg-slate-950/40' : 'bg-slate-100'}`}>
                          <p className={labelCls}>{t('Price')}</p>
                          <p className={`mt-0.5 flex items-center gap-0.5 text-base font-black ${isDark ? 'text-white' : 'text-slate-900'}`}>
                            <IndianRupee className="h-3 w-3 text-emerald-400" />{formatPrice(selFlat.price, formatNumber).replace('₹', '')}
                          </p>
                        </div>
                        <div className={`rounded-lg p-2.5 ${isDark ? 'bg-slate-950/40' : 'bg-slate-100'}`}>
                          <p className={labelCls}>{t('Facing')}</p>
                          <p className={`mt-0.5 text-sm font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{t(selFlat.facing).replace(` ${t('Facing')}`, '')}</p>
                        </div>
                      </div>
                      <div className="rounded-lg border border-emerald-400/20 bg-emerald-500/10 p-2.5">
                        <p className="text-[10px] uppercase tracking-wide text-emerald-400">{t('Status')}</p>
                        <p className="mt-0.5 text-xs font-bold text-emerald-300">{t('Ready to reserve')}</p>
                      </div>
                    </div>
                  ) : (
                    <div className={`mt-2 rounded-lg border border-dashed p-4 text-center ${isDark ? 'border-white/10' : 'border-slate-300'}`}>
                      ...
                      <p className={`mt-1.5 text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{t('Select a green unit to see details.')}</p>
                    </div>
                  )}
                </div>
                ...
                <div className={`mt-3 rounded-lg border p-3 ${isDark ? 'border-cyan-400/15 bg-cyan-400/[.05]' : 'border-sky-200 bg-sky-50/80'}`}>
                  <p className={`flex items-center gap-1 text-[10px] font-semibold ${isDark ? 'text-cyan-200/80' : 'text-sky-700'}`}>
                    <Compass className="h-3 w-3" /> {t('Guide')}
                  </p>
                  <p className={`mt-1 text-xs leading-relaxed ${isDark ? 'text-cyan-50/80' : 'text-slate-700'}`}>{t(selProject.fit)}</p>
                </div>
                ...
                >
                  ...
                  {isSubmitting ? t('Reserving…') : isExpired ? t('Timer Expired') : t('Confirm & Pay')}
                </button>
              </div>
            </div>
          </aside>
      ...
      {showExpandedPlan && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-sm">
          <div className={`relative w-full max-w-4xl rounded-2xl border p-5 shadow-2xl ${isDark ? 'border-slate-700/60 bg-slate-950' : 'border-slate-200 bg-white'}`}>
            ...
            <p className={labelCls}>{t('Expanded Floor Plan')}</p>
            <h3 className={`mt-1 text-xl font-extrabold ${isDark ? 'text-white' : 'text-slate-900'}`}>{selBuilding.name} · {t('Floor')} {selFloorData.number}</h3>
