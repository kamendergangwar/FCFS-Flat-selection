...
const FlatLock = () => {
  ...
  const handleCancelBooking = () => {
    if (confirm(t('Are you sure you want to cancel? Your EMD will be forfeited.'))) {
  ...
  if (!flatLocked) {
    return (
      <ApplicationLayout stepNumber={12} title={t('Flat Lock')}>
        <div className="text-center py-8">
          <AlertCircle className="w-16 h-16 text-amber-500 mx-auto mb-4" />
          <h3 className="text-xl font-semibold text-gray-900 mb-2">{t('No Flat Selected')}</h3>
          <p className="text-gray-600 mb-4">{t('Please select a flat to proceed.')}</p>
          <button
  ...
  return (
    <ApplicationLayout stepNumber={12} title={t('Flat Lock Status')}>
      <div className="space-y-6">
        <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 flex items-start gap-3">
          <Clock className="w-5 h-5 text-amber-600 mt-0.5" />
          <div>
            <p className="font-medium text-amber-800">{t('Flat Locked for 24 Hours')}</p>
            <p className="text-sm text-amber-700">{t('Complete your confirmation payment before the timer expires.')}</p>
          </div>
        </div>

        <div className="rounded-2xl bg-gray-900 p-8 text-center">
          <p className="text-gray-400 text-sm mb-2">{t('Time Remaining')}</p>
          <p className="text-4xl font-bold text-white font-mono">{timeLeft}</p>
        </div>

        <div className="rounded-2xl border border-indigo-200 bg-indigo-50 p-6">
          <h3 className="font-semibold text-gray-900 mb-4">{t('Locked Flat Details')}</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              <p className="text-sm text-gray-500">{t('Flat Number')}</p>
              <p className="font-semibold text-gray-900">{flatLocked.id}</p>
            </div>
            <div>
              <p className="text-sm text-gray-500">{t('Type')}</p>
              <p className="font-semibold text-gray-900">{t(flatLocked.type)}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="font-semibold text-gray-900">₹{formatNumber(flatLocked.price)}</p>
            </div>
          </div>
        </div>
